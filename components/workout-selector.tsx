...
export default function WorkoutSelector({ workouts, onImportWorkout, onImportExercises }: WorkoutSelectorProps) {
  ...
  const filteredWorkouts = workouts.filter(
    (workout) =>
      ...
      workout.items.some((item) => item.name.toLowerCase().includes(search.toLowerCase())),
  )
  ...
  const handleImportExercises = () => {
    if (selectedWorkout) {
      const exercisesToImport = selectedWorkout.items.filter((item) => selectedExercises[item.id])
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}> {/* Updated Dialog and state */}
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
          ...
          <TabsContent value="workouts" className="mt-4">
            <div className="border rounded-md p-2 max-h-[300px] overflow-y-auto">
              {filteredWorkouts.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-[100px] text-center">
                  <p className="text-muted-foreground">No matching workouts found</p>
                </div>
              ) : (
                  ...
                  {filteredWorkouts.map((workout) => (
                    <Card
                      ...
                      className={`cursor-pointer hover:bg-accent transition-colors ${
                        selectedWorkout?.id === workout.id ? "border-primary" : ""
                      }`}
                    ...
                    >
                      <CardHeader className="py-3 px-4">
                        <div className="flex justify-between items-center">
                          ...
                          {selectedWorkout?.id === workout.id && <CheckIcon className="h-5 w-5 text-primary" />}
                        </div>
                      </CardHeader>
            ...
            {selectedWorkout && (
              <div className="mt-4 flex justify-end">
                <Button onClick={handleImportWorkout}>Import Entire Workout</Button>
              </div>
            )}
          </TabsContent>
            ...
            {selectedWorkout ? (
              <>
                <div className="border rounded-md p-2 max-h-[300px] overflow-y-auto">
                  <div className="space-y-2">
                    {selectedWorkout.items.map((exercise) => (
                      <div key={exercise.id} className="flex items-center space-x-3 p-3 rounded hover:bg-accent">
                        <Checkbox
                        ...
                        <div className="grid grid-cols-3 gap-2 flex-1">
                          ...
                          <span className="text-sm text-muted-foreground">{exercise.repeats} reps</span>
                          <span className="text-sm text-muted-foreground">{exercise.weight} kg</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <Button onClick={handleImportExercises} disabled={selectedCount === 0}>
        ...
        <DialogFooter className="mt-4"> {/* Updated DialogFooter */}
          <Button variant="outline" onClick={() => setOpen(false)}> {/* Updated onClick handler */}
            Cancel
          </Button>
        </DialogFooter>
      </DialogContent>
